...
module.exports = {
  ...
  add: async (values) => {
    ...
    const relations = _.pick(values, Candidat.associations.map(ast => ast.alias));
    const data = _.omit(values, Candidat.associations.map(ast => ast.alias));
  ...
  edit: async (params, values) => {
    ...
    // Create relational data and return the entry.
    return Candidat.updateRelations(Object.assign(params, { values: relations }));
  },
  ...
  search: async (params) => {
    ...
    const searchText = Object.keys(Candidat._attributes)
      .filter(attribute => attribute !== Candidat.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['string', 'text'].includes(Candidat._attributes[attribute].type));

    const searchNoText = Object.keys(Candidat._attributes)
      .filter(attribute => attribute !== Candidat.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Candidat._attributes[attribute].type));

    const searchInt = Object.keys(Candidat._attributes)
      .filter(attribute => attribute !== Candidat.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['integer', 'decimal', 'float'].includes(Candidat._attributes[attribute].type));

    const searchBool = Object.keys(Candidat._attributes)
      .filter(attribute => attribute !== Candidat.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['boolean'].includes(Candidat._attributes[attribute].type));
