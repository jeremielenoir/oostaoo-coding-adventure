...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Offer.query(function(qb) {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Offer._attributes)
      .filter(attribute => attribute !== Offer.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Offer._attributes[attribute].type));
