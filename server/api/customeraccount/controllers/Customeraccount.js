...
module.exports = {
  ...
  saveCard: async (ctx) => {
    ...
    try {
      return strapi.services.customeraccount.createCustomer(
        ctx.request.body,
        account,
        ctx.state.user,
      );
  ...
  offerCreate: async (ctx) => {
    try {
      ...
      if (paymentIntentId) {
        ...
        intent = await strapi.services.customeraccount.finalizePaymentIntent(
          ctx.request.body,
        );
      } else {
      ...
      const parsedIntent =
        strapi.services.customeraccount.parsePaymentIntent(intent);

      strapi.log.info(`parsed intent for account ${account.id}`, parsedIntent);
      ctx.send(parsedIntent);

      if (parsedIntent.done) {
        await strapi.services.customeraccount.onPaymentSucceed(
          intent,
          account,
          ctx.state.user,
        );
      ...
      ctx.body = {
        status: 'error',
        message: { code: 'stripe_paiement', message: e.message },
      };
  ...
  offerUpdate: async (ctx) => {
    // fetch needed datas
    let account = ctx.request.body.customeraccount;
    ...
    account = await strapi.services.customeraccount.edit(
      { id: account.id },
      { offer: offer.id },
    );
  ...
  offerDestroy: async (ctx) => {
    try {
      ...
      if (!offer || offer.get('periodicity') !== 'monthly') {
        ctx.badRequest(null, [
          { messages: [{ id: 'No subscription to cancel' }] },
        ]);
        return;
      }
      ...
      if (!payments || payments.length === 0) {
        ...
        ctx.badRequest(null, [
          { messages: [{ id: 'No payment found for subscription' }] },
        ]);
        return;
      }
  ...
  userUpdate: async (ctx) => {
    ...
    const userFromDb = await strapi.plugins[
      'users-permissions'
    ].services.user.fetch({ id: userIdToEnable });
    ...
    if (userFromDb.customeraccount.id === accountId) {
      ...
      ctx.body = {
        status: 'error',
        message: { code: 'error', message: 'fail to update user' },
      };
    }
  },
  /**
  ...
  userFind: async (ctx) => {
    ...
    results.forEach((u) => {
      users.push(
        _.omit(u.toJSON ? u.toJSON() : u, ['password', 'resetPasswordToken']),
      );
    });
  ...
  userCreate: async (ctx) => {
    try {
      ...
      if (!ctx.request.body.role) {
        const defaultRole = await strapi.plugins[
          'users-permissions'
        ].services.role.fetch({ type: advanced.default_role }, []);
  ...
  userDestroy: async (ctx) => {
    ...
    const userFromDb = await strapi.plugins[
      'users-permissions'
    ].services.user.fetch({ id: userToRemoveId });
    ...
    if (userFromDb.customeraccount.id === accountId) {
      ...
      ctx.body = {
        status: 'error',
        message: { code: 'error', message: 'fail to remove user' },
      };
    }
  },
  /**
