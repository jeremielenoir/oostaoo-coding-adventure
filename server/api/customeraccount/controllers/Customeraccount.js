...
module.exports = {
    ...
    userFind: async(ctx) => {
        let users = [];
        let account = ctx.state.user.customeraccount;
        const results = await strapi.plugins['users-permissions'].services.user.fetchAll({ customeraccount: account.id });

        results.forEach(u => { users.push(_.omit(u.toJSON ? u.toJSON() : u, ['password', 'resetPasswordToken'])); });
        ...
        try {
            ...
            if (advanced.unique_email && ctx.request.body.email) {
                ...
                if (users && users.length > 0) {
                    return ctx.badRequest(null, ctx.request.admin ? [{ messages: [{ id: 'Auth.form.error.email.taken', field: ['email'] }] }] :
