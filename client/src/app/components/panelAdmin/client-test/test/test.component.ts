...
export class TestComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(AlgoComponent) algoComponent: AlgoComponent;
  ...
  public openDialogTimeout(preview: boolean = true): void {
    ...
    dialogRef.componentInstance.questNext.subscribe((item) =>
      this.questNext(item),
    );
  ...
  public checkCheckBoxvalue(event) {
    if (event.source.checked) {
      ...
      const element = this.arrayReponseUser.find(
  ...
  public questNext(event: any) {
    ...
    if (this.checkTimeDefault === false) {
      this.arrayGoodRep = this.question.answer_value
  ...
  public checkRep() {
    switch (this.questions[this.index].type) {
      case 'one':
        ...
        if (
          this.arrayGoodRep.sort().toString() ===
          this.arrayReponseUser.sort().toString()
        ) {
          console.log('ITS OK !!');
          this.counterCheck++;
          this.sumPointsByRepCandidat(
            this.questions[this.index].technologies,
      ...
      case 'free':
        ...
        if (
          this.arrayReponseUser.every((reps) =>
            this.arrayGoodRep.includes(reps),
          )
        ) {
          console.log('item ok !');
          this.counterCheck++;
      ...
      case 'multiple':
        ...
        break;
      case 'algo':
        this.algoComponent.testCode();
        if (this.algoComponent.responseTestAnswer) {
          console.log('ITS OK !!');
          this.counterCheck++;
          this.sumPointsByRepCandidat(
            this.questions[this.index].technologies,
              ...
              .subscribe((res2) => {
                ...
                this.apiClientService
                  .post(API_URI_NOTIFICATIONS, {
                    ...
                    status: false,
                    title:
                      "Un candidat viens de finir le test '" +
                      res.campaign.Name +
                      "'",
  ...
  public postRapportCandidat() {
    ...
    this.apiClientService
      .get(API_URI_CANDIDATS + '/' + this.candidat.id)
      .toPromise()
      .then((res) => {
        if (res.raport_candidat !== null) {
          this.jsonRapport = res.raport_candidat;
          console.log('JSON : ', this.jsonRapport);
        }
  ...
  public sumPointsByRepCandidat(techno, point) {
    this.apiClientService
      ...
      .toPromise()
      .then((res) => {
        console.log('sumPointsByRepCandidat: ', res);
  ...
  public calculTotalPoints(array) {
    if (typeof array !== 'undefined' && array.length > 0) {
      this.totalPoints = array.reduce((a, b) => ({
