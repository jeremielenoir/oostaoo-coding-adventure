...
import { ApiClientService, API_URI_ADDRESS } from 'src/app/api-client/api-client.service';
...
export class AddressComponent implements OnInit {
  ...
  constructor(
    ...
    this.addressForm = this.formBuilder.group({
      ...
      postal_code: [this.address ? this.address.postal_code : null, Validators.required],
      city: [this.address ? this.address.city : null, Validators.required],
      state: [this.address ? this.address.state : null],
      country: [this.address ? this.address.country : null, Validators.required]
    });
  ...
  onSubmit(addressData) {
    ...
    if (this.address) {
      ...
      this.apiClientService
      .post(API_URI_ADDRESS, addressData)
      .subscribe(
        (data) => {
          this.dialogRef.close(data);
        },
